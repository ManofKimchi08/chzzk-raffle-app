...
네이버 **치지직(CHZZK)** 라이브 방송 채팅창과 웹소켓(WebSocket)으로 직접 연결하여, **단독 시작 화면(스트리머 이름 검색 & 팔로워 순 정렬)**, **수천 명 단위 시청자 공정 추첨**, **비주얼 룰렛**, **실시간 채팅/치즈 투표**, 시청자들과 실시간 채팅으로 대결하는 **포켓몬 배틀(👑 스트리머 vs 시청자 & 🔴 시청자 진영 대항전 🔵)**, **포켓몬 퀴즈 쇼(실루엣/도감/울음소리/초성)**, **찌리리공 뒤집기**, 그리고 **도킹 실시간 채팅창**까지 완벽 지원하는 올인원 방송 인터랙션 플랫폼입니다.
...
| **05. 포켓몬 배틀 & 퀴즈 쇼 엔진 명세서** | 258마리 포켓몬 데이터, 진영 대항전, 데미지/상성 공식, 메가진화, 4대 퀴즈, 블라인드 투표 | [🎮 게임 엔진 바로가기](docs/05_pokemon_battle_and_quiz_engine_ko.md) |
...
### 1. 👥 포켓몬 배틀 「시청자 진영 대항전 (레드팀 vs 블루팀)」 신설
- **시청자 vs 시청자 두뇌 대결**: 스트리머와 시청자가 맞붙던 기존 모드(`👑 스트리머 vs 시청자`)에 더해, 방송 참가자들이 두 팀으로 나뉘어 각자의 포켓몬을 지휘하는 `🔴 👥 시청자 진영 대항전 🔵` 공식 신설.
- **원클릭 모드 전환**: 대기실 및 좌측 환경설정 서랍에서 대전 방식을 즉시 변경 가능하며, 진영 이름(`레드팀` / `블루팀`)과 모집 시간(15~60초)을 자유롭게 커스텀할 수 있습니다.

### 2. 🚩 시청자 직접 진영 선택 모드 vs 🎲 50:50 자유 채팅 무작위 추첨
- **🚩 시청자 직접 진영 선택 (Viewer Choice Mode)**: 시청자가 채팅창에 원하는 팀 키워드(`1`/`레드` vs `2`/`블루`)를 입력해 가입할 진영을 결정하며, 실시간 2단 버킷 UI와 자유로운 팀 이적을 지원합니다.
- **🎲 50:50 자유 채팅 무작위 배정 (Random Equal Mode)**: 일상 채팅 아무거나 치면 접수되며, Fisher-Yates 셔플 알고리즘으로 양 진영에 50:50 황금 밸런스로 배정됩니다.

![시청자 직접 진영 선택 모집 모달](docs/images/faction_choice_recruitment_modal.png)
![시청자 직접 진영 선택 완료 모달](docs/images/faction_choice_reveal_modal.png)

### 3. ⚖️ 기술창 완벽 좌우 대칭 디자인 & 높이 1:1 일치 (Symmetrical Move Decks)
- **전폭 다이얼로그 & 단일 턴 타이머 바**: 상단 배틀 상황 메시지와 턴 타이머 바를 단일 전폭(`100%`) 바 형태로 통합.
- **1:1 대칭 그리드 (`1fr 1fr`)**: 레드팀과 블루팀 기술창 높이를 278px로 완벽히 동일화(오차 0px).
- **2열 페어링 그리드**: 일반 기술(`1`~`4`)과 메가진화 기술(`메가1`~`4`), 교체 행동(`5`, `6`)을 2열 분할 카드로 배치하여 완벽한 시각적 균형감 완성.

![완벽한 좌우 대칭 기술창 디자인](docs/images/symmetrical_faction_action_decks.png)

### 4. 🔍 시청자 내전 VIP / 전광판 / 명단 / 영광의 참가자 원클릭 UID & 채팅 인스펙터
- **전광판 칩 클릭 인스펙터**: 전광판 속 닉네임 칩을 클릭하면 해당 시청자의 고유 UID, 원클릭 복사 버튼, 후원/구독 뱃지, 실시간 채팅 내역 모달 즉시 팝업.
- **승리 결과 모달 '영광의 참가자' 인터랙티브 칩**: 결과창의 승리 팀원 전원을 골드 테두리 인터랙티브 칩(`glory-user-chip`)으로 개편하여 클릭 시 상세 인스펙터 호출 및 `[📋 전체 명단 & 검색]` 지원.
- **3중 레이어 z-index 스택 아키텍처**: 결과 모달(`1100`) ➔ 진영 명단 모달(`1150`) ➔ UID 인스펙터(`1250`) 순으로 충돌 없이 부드럽게 오버레이.

![전광판 VIP 칩 클릭 시 UID 인스펙터](docs/images/vip_billboard_uid_inspector.png)
![승리 결과 모달 영광의 참가자 골드 칩](docs/images/glory_participants_result_modal.png)
![결과창 위에서 팝업된 UID 인스펙터](docs/images/glory_participant_uid_inspect.png)

### 5. 🧬 메가진화 선택 번호(`🧬 메가1~4`) 명시 & 실시간 채팅 투표 연동
- **메가진화 카드 라벨 번호 삽입**: 기존 번호 없던 `🧬 [메가] 화염방사` ➔ **`🧬 메가1. 화염방사`**, **`🧬 메가2. 불대문자`** 등 선택 번호 명시.
- **HUD 상단 안내 자막 동적 실시간 반영**: 3v3 메가진화 가능 시 `(채팅: 1~4 / 메가1~4 / 5,6)`로 자동 표시.
- **채팅 투표 파서 강화 & 숫자 단축키 지원**: `메가1`~`메가4`, `M1`~`M4`, `1메가`~`4메가`, `메가1번`~`메가4번` 및 숫자 단축키 `7~10` (1v1 모드는 `5~8`) 전면 지원.

![진영 대항전 메가진화 번호 표시 및 대칭 카드](docs/images/faction_battle_mega_selection_numbers.png)
![1v1 시청자 투표 모드 메가진화 번호 및 가이드](docs/images/poke_1v1_mega_selection_numbers.png)

### 6. 🎬 양측 영화 크레딧 롤링 전광판 & 실시간 투표 네온 발광 (Live Vote Glow)
- **영화 엔딩 크레딧 스크롤**: 경기장 좌우측에 사이버 스타디움 감성의 전광판을 배치하고, 상하단 페이드 마스크를 적용하여 팀원 닉네임이 엔딩 크레딧처럼 부드럽게 무한 순환 하강합니다.
- **실시간 투표 네온 발광 (Live Vote Glow)**: 팀원이 채팅으로 `1`~`4` 기술이나 `5`,`6` 교체를 투표하면, 전광판 속 본인 닉네임 칩이 황금빛 네온(⚡)으로 반짝이며 선택한 행동 뱃지가 즉각 표시됩니다.
- **지각생 자동 밸런싱**: 배틀 도중 뒤늦게 투표를 입력한 시청자도 인원수가 적은 팀 전광판으로 자동 편입되어 균형을 유지합니다.

![배틀 아레나 양측 영화 크레딧 전광판 및 진영 대항전](docs/images/faction_battle_arena_billboards.png)

### 7. 🎙️ 중계자(스트리머) 제어 센터 & 독립 투표 HUD
- **스트리머 캐스터 바**: 경기장 상단에서 `[ ⏭️ 즉시 개표 & 배틀 진행 ]`, `[ ⏱️ +10초 ]`, `[ 👥 진영 명단 ]` 버튼으로 방송 진행 속도를 완벽 제어.
- **양측 독립 투표 HUD**: 좌측(레드팀), 우측(블루팀)에 각각 분리된 실시간 투표 HUD를 통해 양 팀의 표심을 실시간으로 중계할 수 있습니다.

![중계자 진영 명단 확인 모달](docs/images/faction_roster_modal.png)
