# 🎮 치지직 대규모 시청자 추첨 & 룰렛 & 실시간 투표 & 포켓몬 배틀 플랫폼 (Chzzk Interactive Hub) v4.5.0

![Chzzk WebSocket Live](https://img.shields.io/badge/Chzzk-WebSocket%20Live-00ffa3?style=for-the-badge&logo=naver)
![Version](https://img.shields.io/badge/Release-v4.5.0-purple?style=for-the-badge)
...
## 🌟 v4.5.0 주요 신규 및 개선 사항 (What's New in v4.5.0)

### 1. ⚔️ 포켓몬 배틀 스트리머 & 시청자 동시 턴 선택 시스템 (Simultaneous Turn)
- **단일 카운트다운 동안 동시 진행**: 스트리머가 선택 후 시청자가 기다리던 순차 방식을 완전히 개편하여, 단일 턴 제한 시간(10초~60초) 동안 **스트리머(우측 덱 클릭)와 시청자(좌측 실시간 채팅 투표)가 동시에 행동을 선택**합니다.
- **실시간 선택 상태 피드백 & 하이라이트**: 스트리머가 기술/교체를 클릭하면 초록색 네온 테두리(`✓`) 및 상태 배너(`👑 스트리머 선택 완료`)가 즉시 표시되며, 타이머 종료 전까지 자유롭게 변경할 수 있습니다.
- **⚡ 즉시 배틀 진행 지원**: 스트리머가 선택을 마치고 시청자 투표가 충분히 모였을 경우, 남은 시간을 기다리지 않고 바로 전투 시퀀스로 진입할 수 있는 `[⚡ 즉시 배틀 진행 (남은 시간 생략)]` 버튼을 제공합니다.

### 2. ⏱️ 턴 제한 시간 UI 단일 통합
- 상단 요약 바 및 좌측 서랍 설정 패널에서 개별로 나뉘어 있던 설정 항목들을 직관적인 **`⏱️ 턴 제한 시간 (동시 선택)`** 단일 컨트롤로 통합하여 한결 깔끔하고 편리해졌습니다.

### 3. 💀 3v3 포켓몬 기절 교체 시스템 고도화
- 포켓몬이 쓰러졌을 때 열리는 **스트리머 출전 선택 모달**과 **시청자 연합 출전 투표 모달**이 설정된 시간과 실시간으로 정밀하게 연동되며, 교체 투입 후 바로 다음 라운드의 동시 턴으로 자연스럽게 전환됩니다.
...
- **213마리 챔피언스 풀 & 14종 지닌 도구 & 46종 메가진화**: 구애시리즈, 기합의띠, 돌격조끼, 생명의구슬, 자뭉열매, 먹밥, 메가스톤 등 완벽 구현
