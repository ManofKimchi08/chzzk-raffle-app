...
### 2. 💬 자유 채팅 자동 편입 (Omni-Chat Auto-Enrollment)
- **아무 말이나 치면 자동 등록**: 복잡한 명령어 없이 모집 시간 동안 시청자가 채팅창에 `"ㅋㅋ"`, `"안녕하세요"`, `"가즈아"`, `"1"` 등 아무 채팅이나 치면 치지직 UID 기준으로 자동 등록됩니다.
- **중복 방지 & 실시간 카운터**: 중복 참여는 자동으로 필터링되며, 참가자 수와 닉네임 뱃지가 실시간으로 활기차게 추가됩니다.

![시청자 진영 대항전 참가자 모집 모달](docs/images/faction_recruitment_modal.png)

### 3. ⚖️ 50:50 황금 밸런스 무작위 셔플 (Fisher-Yates Equal Draft)
- **공정한 50:50 배분**: Fisher-Yates 셔플 알고리즘을 통해 수집된 참가자를 양 진영에 정확히 50:50으로 무작위 균등 배정합니다.
- **가상 시청자 자동 보충**: 참가자가 2명 미만일 경우 가상 시청자(시청자1~6)를 자동으로 채워 넣어 중단 없이 배틀을 즐길 수 있습니다.

![50:50 무작위 균등 배정 완료 모달](docs/images/faction_reveal_modal.png)

### 4. 🎬 양측 영화 크레딧 롤링 전광판 & 실시간 투표 네온 발광 (Live Vote Glow)
...
![실시간 채팅 투표 및 전광판 네온 발광 효과](docs/images/faction_battle_turn_voting.png)

### 5. 🎙️ 중계자(스트리머) 제어 센터 & 독립 투표 HUD
